% Eliza chatbot main entry point
eliza:- 
    writeln('Hola , mi nombre es  Eliza tu  chatbot, por favor ingresa tu consulta, usar solo minúsculas sin . al final:'),
    readln(Input),
    eliza(Input), !.

% Eliza chatbot exit condition
eliza(Input):- 
    Input == ['Adios'],
    writeln('Adios. espero poder verte ayudado.'), !.

% Eliza chatbot exit condition with period
eliza(Input):- 
    Input == ['Adios', '.'],
    writeln('Adios. espero poder verte ayudado.'), !.

% Eliza chatbot main processing
eliza(Input) :-
...
% Templates for matching input and generating responses
...
elizaDoes(X, R):- 
    does(X), 
    R = ['Yes', i, X, and, i, love, it].
elizaDoes(X, R):- 
    \+does(X), 
    R = ['No', i, do, not, X ,'.', it, is, too, hard, for, me].
does(study).
does(cook).
does(work).
...
% Replacing placeholders in responses
replace0([], _, _, Resp, R):- 
    append(Resp, [], R),!.
